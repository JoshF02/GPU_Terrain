...
public:
    ...
    //int perm[256 * 2];
    const int perm[512] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225,
    ...
    double Perlin(double x, double y) {
        ...
        int x0y0 = perm[perm[X] + Y];
        int x0y1 = perm[perm[X] + Y + 1];
        int x1y0 = perm[perm[X + 1] + Y];
        int x1y1 = perm[perm[X + 1] + Y + 1];

        return lerp(v,
            lerp(u, grad(x0y0, x, y), grad(x1y0, x - 1, y)),
            lerp(u, grad(x0y1, x, y - 1), grad(x1y1, x - 1, y - 1))
        );
    }
    ...
    double Simplex(double x, double y) {
        ...
        double s = (x + y) * F2;    // step 1
        int i = int(floor(x + s));
        int j = int(floor(y + s));

        double t = double(i + j) * G2;  // step 3
        ...
        double x1 = x0 - double(i1) + G2;   // step 4 continued
        ...
        double t0 = 0.5 - x0 * x0 - y0 * y0;  // steps 5 and 6  
        if (t0 < 0.0) n0 = 0.0;
        else {
            t0 *= t0;
